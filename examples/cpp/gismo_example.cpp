...
bool
equal_buf(void *buf1, void *buf2, size_t len) {
...
void
printParams(const nixl_b_params_t &params, const nixl_mem_list_t &mems) {
    if (params.empty()) {
...
main() {
    ...
    if (!found) {
        std::cerr << "Not found gismo plugin, exit" << std::endl;
        return -1;
    }
    std::cout << "Found gismo plugin" << std::endl;
    ret1 = A1.getPluginParams("gismo", mems1, init1);
    ret2 = A2.getPluginParams("gismo", mems2, init2);
    ...
    std::string socketPath = getenv("DMO_SOCKET_PATH");
    if (socketPath.empty()) {
        socketPath = "dmo.daemon.sock.sheperd";
    }
    init1["socket_path"] = socketPath;
    init2["socket_path"] = socketPath;

    nixlBackendH *gismo1, *gismo2;
    ret1 = A1.createBackend("Gismo", init1, gismo1);
    ret2 = A2.createBackend("Gismo", init2, gismo2);
    assert(ret1 == NIXL_SUCCESS);
    assert(ret2 == NIXL_SUCCESS);
    std::cout << "Created backend sucessfully" << std::endl;

    nixl_opt_args_t extra_params1, extra_params2;
    extra_params1.backends.push_back(gismo1);
    extra_params2.backends.push_back(gismo2);

    assert(ret1 == NIXL_SUCCESS);
    assert(ret2 == NIXL_SUCCESS);

    ret1 = A1.getBackendParams(gismo1, mems1, init1);
    ret2 = A2.getBackendParams(gismo2, mems2, init2);
    ...
    std::cout << "Got ret " << ret1 << std::endl;
    assert(ret1 == NIXL_SUCCESS);
    assert(ret2 == NIXL_SUCCESS);
    ...
    std::cout << "Write Transfer verified\n";

    ret1 = A1.releaseXferReq(req_handle);
    assert(ret1 == NIXL_SUCCESS);

    std::cout << "Ready to do Read Transfer\n";
    ret1 = A1.createXferReq(
        NIXL_READ, req_src_descs, req_dst_descs, agent2, req_handle, &extra_params1);
    std::cout << "Got ret " << ret1 << std::endl;
    assert(ret1 == NIXL_SUCCESS);

    status = A1.postXferReq(req_handle);

    std::cout << "Read Transfer was posted\n";

    while (status != NIXL_SUCCESS) {
        if (status != NIXL_SUCCESS) status = A1.getXferStatus(req_handle);
        assert(status >= 0);
    }
    std::cout << "Read Transfer verified\n";
    ret1 = A1.releaseXferReq(req_handle);
    assert(ret1 == NIXL_SUCCESS);
