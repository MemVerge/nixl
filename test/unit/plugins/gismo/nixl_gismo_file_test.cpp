...
public:
    ...
    tempFile &
    operator=(tempFile &&other) noexcept {
        if (this != &other) {
            close_fd();
...
read_write_test(int num_transfers,
    ...
    nixlAgent agent("gismoFileReadWriteTester", nixlAgentConfig(true));
    ...
    print_segment_title("NIXL STORAGE WRITE/READ TEST STARTING (Gismo PLUGIN)");
    ...
    // Create Gismo backend first - before allocating any resources
    nixlBackendH *posix = nullptr;
    nixl_status_t status = agent.createBackend("Gismo", params, posix);
    if (status != NIXL_SUCCESS) {
        ...
        std::cerr << "Error creating Gismo backend: " << nixlEnumStrings::statusStr(status)
                  << std::endl;
        std::cerr << std::endl << line_str << std::endl;
        return 1;
    }
    ...
    try {
        ...
        status = agent.createXferReq(
            NIXL_WRITE, dram_for_posix_xfer, file_for_posix_xfer, "gismoFileReadWriteTester", treq);
        if (status != NIXL_SUCCESS) {
        ...
        status = agent.createXferReq(
            NIXL_READ, dram_for_posix_xfer, file_for_posix_xfer, "gismoFileReadWriteTester", treq);
        if (status != NIXL_SUCCESS) {
...
int
test_posix_repost(std::string test_files_dir_path_abs_path, std::string socket_path) {
    ...
    params["socket_path"] = socket_path;
    print_segment_title("NIXL STORAGE REPOST TEST STARTING (Gismo PLUGIN)");

    // Create POSIX backend first - before allocating any resources
    nixlBackendH *posix = nullptr;
    nixlAgent agent("gismoFileRepostTester", nixlAgentConfig(true));
    if (agent.createBackend("Gismo", params, posix) != NIXL_SUCCESS) {
        std::cerr << "Failed to create Gismo backend" << std::endl;
        return 1;
    }
    ...
    nixl_status_t status = agent.createXferReq(
        NIXL_WRITE, dram_for_posix_xfer, file_for_posix_xfer, "gismoFileRepostTester", treq_write);
    if (status != NIXL_SUCCESS) {
        std::cerr << "Failed to create write transfer request - status: "
                  << nixlEnumStrings::statusStr(status) << std::endl;
        return 1;
    ...
    status = agent.createXferReq(
        NIXL_READ, dram_for_posix_xfer, file_for_posix_xfer, "gismoFileRepostTester", treq_read);
    if (status != NIXL_SUCCESS) {
        std::cerr << "Failed to create read transfer request - status: "
                  << nixlEnumStrings::statusStr(status) << std::endl;
        return 1;
...
main(int argc, char *argv[]) {
    ...
    std::cout << "NIXL Gismo Plugin File Test" << std::endl;
    ...
    while ((opt = getopt(argc, argv, "n:s:d:hp:")) != -1) {
        switch (opt) {
        case 'n':
    ...
    int ret =
        read_write_test(num_transfers, transfer_size, test_files_dir_path_abs_path, socket_path);
    ...
    ret = test_posix_repost(test_files_dir_path_abs_path, socket_path);
    if (ret != 0) {
