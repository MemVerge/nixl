...
static const char *PLUGIN_NAME = "Gismo";
static const char *PLUGIN_VERSION = "0.1.0";

// Function to create a new gismo backend engine instance
static nixlBackendEngine *
create_gismo_engine(const nixlBackendInitParams *init_params) {
    return new nixlGismoEngine(init_params);
}

static void
destroy_gismo_engine(nixlBackendEngine *engine) {
    delete engine;
}
...
get_backend_options() {
    ...
    params["socket_path"] = "";
    params["client_log_path"] = "/tmp/mvfs_client.log";
...
static nixlBackendPlugin plugin = {NIXL_PLUGIN_API_VERSION,
                                   create_gismo_engine,
                                   destroy_gismo_engine,
                                   get_plugin_name,
                                   get_plugin_version,
                                   get_backend_options,
                                   get_backend_mems};

#ifdef STATIC_PLUGIN_gismo

nixlBackendPlugin *
createStaticGismoPlugin() {
